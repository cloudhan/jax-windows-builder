...
try {
    ...
    $disk_cache = Join-Path (pwd) "bazel-build-cache" "$cuda_tag"
    ...
    mkdir "bazel-win-dist/$cuda_tag" -ErrorAction 0
    mv -Force "dist/${name}" "bazel-win-dist/$cuda_tag/$new_name"
}
finally {
    Pop-Location
    Pop-Environment
